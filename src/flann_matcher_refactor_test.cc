  ...
  for (int idx = 0; idx < org_matches.size(); idx++) {
    ...
    for (int idx_i = 0; idx_i < org.size(); idx_i++) {
      ...
      if (org_match.distance != ref_match.distance) {
        LOG(INFO) << "Original : " << org_match.distance;
        LOG(INFO) << "Refactored : " << ref_match.distance;
...
int main(int argc, char** argv) {
  ...
  // 2. Key Point Compare with Flann Based Matcher.
  std::vector<std::vector<cv::DMatch>> org_matches;
  {
    cv::setRNGSeed(0);
    bool cross_check = false;
    cv::Ptr<cv::FlannBasedMatcher> matcher = cv::FlannBasedMatcher::create();
    matcher->knnMatch(descriptors1, descriptors2, org_matches, 2, cv::noArray());
    LOG(INFO) << "Matched keypoints (FLANN Matcher) : " << org_matches.size();
  }

  // 3. Key Point Compare with Refactored Flann Based Matcher.
  std::vector<std::vector<cv::DMatch>> ref_matches;
  {
    cv::setRNGSeed(0);
    cv::Ptr<cv_copy::FlannBasedMatcher> matcher = cv_copy::FlannBasedMatcher::create();
    matcher->knnMatch(descriptors1, descriptors2, ref_matches, 2, cv::noArray());
    LOG(INFO) << "Matched keypoints (FLANN Matcher) : " << ref_matches.size();
  }
